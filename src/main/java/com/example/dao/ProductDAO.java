...
// Lớp này chứa các phương thức thao tác với bảng "products" trong cơ sở dữ liệu
public class ProductDAO {
    
    /**
     * Lấy danh sách tất cả sản phẩm (không lọc theo điều kiện nào)
     */
    public List<Product> getAllProducts() {
        List<Product> products = new ArrayList<>();
        String sql = "SELECT * FROM products ORDER BY id DESC"; // Sắp xếp theo id giảm dần

        try (Connection conn = DatabaseConnection.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {

            // Duyệt qua kết quả và gán dữ liệu vào đối tượng Product
            while (rs.next()) {
                ...
                product.setStockQuantity(rs.getInt("stock_quantity")); // Số lượng tồn kho
    ...
    /**
     * Tìm kiếm sản phẩm theo từ khóa (keyword) trong tên, mô tả hoặc thương hiệu
     */
    ...
    /**
     * Cập nhật sản phẩm (bao gồm hình ảnh)
     */
    ...
    /**
     * Cập nhật sản phẩm không bao gồm hình ảnh
     */
    ...
    /**
     * Cập nhật tồn kho (stock_quantity) cho sản phẩm
     */
    ...
    /**
     * Lấy sản phẩm theo category ID (chỉ lấy một số thông tin cơ bản)
     */
    ...
    /**
     * Lấy danh sách sản phẩm sắp xếp theo giá (tăng/giảm)
     */
    ...
    /**
     * Lọc sản phẩm theo nhiều tiêu chí: từ khóa, giá, sắp xếp
     */
    ...
    /**
     * Lọc sản phẩm theo keyword, giá, sắp xếp và có thêm điều kiện danh mục (category)
     */
